<?php
declare(strict_types = 1);                                // 맨위
require './includes/database-connection.php';              // 현재경로
require 'includes/functions.php';                           

$term  = filter_input(INPUT_GET, 'term');                 // 검색어
...
if ($term) {                                              // If search term provided
    $arguments['term1'] = '%' . $term .'%';               // title Like 
    $arguments['term2'] = '%' . $term .'%';               // summary Like
    $arguments['term3'] = '%' . $term .'%';               // content LIKE
    ...
    $count = pdo($pdo, $sql, $arguments)->fetchColumn();  // 결과행 갯수

    if ($count > 0) {                                     // If articles match term
        $arguments['show'] = $show;                       // 3
        $arguments['from'] = $from;                       // 0
...
<main class="container" id="content">
    <section class="header">
      ...
      <?php if ($term) { ?><p><b>검색 결과:</b> <?= $count ?></p><?php } ?>
    </section>
